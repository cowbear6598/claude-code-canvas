...
export function appendToolToLastSubMessage(subMessages: SubMessage[], toolUseInfo: ToolUseInfo): SubMessage[] {
...
export function flushAndCreateNewSubMessage(subMessages: SubMessage[], messageId: string, toolUseInfo: ToolUseInfo): SubMessage[] {
    const updated = [...subMessages]
    const lastIndex = updated.length - 1
    const lastSub = updated[lastIndex]
...
function updateLastSubMessage(subMessages: SubMessage[], delta: string, isPartial: boolean): SubMessage[] {
...
export function updateAssistantSubMessages(
    ...
    const subMessages = updateLastSubMessage(existingMessage.subMessages, delta, isPartial)
    return { subMessages }
}

export function markToolWithOutput(toolUse: ToolUseInfo[], toolUseId: string, output: string): ToolUseInfo[] {
...
function mergeEmptySubMessages(subMessages: SubMessage[]): SubMessage[] {
    ...
    for (const sub of subMessages) {
        ...
        if (isEmpty && hasTool && result.length > 0) {
            ...
            const newTools = sub.toolUse!.filter(t => !existingIds.has(t.toolUseId))
