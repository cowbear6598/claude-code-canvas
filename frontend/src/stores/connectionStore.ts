...
export const useConnectionStore = defineStore('connection', () => {
    ...
    const getConnectionsByPodId = computed(() => (podId: string): Connection[] => {
        return connections.value.filter(
            connection => connection.sourcePodId === podId || connection.targetPodId === podId
        )
    })

    const getOutgoingConnections = computed(() => (podId: string): Connection[] => {
        return connections.value.filter(connection => connection.sourcePodId === podId)
    })

    const getConnectionsByTargetPodId = computed(() => (podId: string): Connection[] => {
        return connections.value.filter(connection => connection.targetPodId === podId)
    })

    const selectedConnection = computed((): Connection | null => {
        if (!selectedConnectionId.value) return null
        return connections.value.find(connection => connection.id === selectedConnectionId.value) || null
    })

    const isSourcePod = computed(() => (podId: string): boolean => {
        return !connections.value.some(connection => connection.targetPodId === podId)
    })
    ...
    const getAiDecideConnectionsBySourcePodId = computed(() => (sourcePodId: string): Connection[] => {
        return connections.value.filter(
    ...
    const getPodWorkflowRole = computed(() => (podId: string): WorkflowRole => {
        const hasUpstream = connections.value.some(connection => connection.targetPodId === podId)
        const hasDownstream = connections.value.some(connection => connection.sourcePodId === podId)
    ...
    function updateAutoGroupStatus(targetPodId: string, status: ConnectionStatus): void {
    ...
    function setConnectionStatus(connectionId: string, status: ConnectionStatus): void {
    ...
    function getWorkflowEventMap(): Array<[string, (payload: unknown) => void]> {
        return [
            [WebSocketResponseEvents.WORKFLOW_AUTO_TRIGGERED, castHandler(handleWorkflowAutoTriggered)],
            [WebSocketResponseEvents.WORKFLOW_COMPLETE, castHandler(handleWorkflowComplete)],
            [WebSocketResponseEvents.WORKFLOW_AI_DECIDE_PENDING, castHandler(handleAiDecidePending)],
            [WebSocketResponseEvents.WORKFLOW_AI_DECIDE_RESULT, castHandler(handleAiDecideResult)],
            [WebSocketResponseEvents.WORKFLOW_AI_DECIDE_ERROR, castHandler(handleAiDecideError)],
            [WebSocketResponseEvents.WORKFLOW_AI_DECIDE_CLEAR, castHandler(handleAiDecideClear)],
            [WebSocketResponseEvents.WORKFLOW_AI_DECIDE_TRIGGERED, castHandler(handleWorkflowAiDecideTriggered)],
            [WebSocketResponseEvents.WORKFLOW_DIRECT_TRIGGERED, castHandler(handleWorkflowDirectTriggered)],
            [WebSocketResponseEvents.WORKFLOW_DIRECT_WAITING, castHandler(handleWorkflowDirectWaiting)],
            [WebSocketResponseEvents.WORKFLOW_QUEUED, castHandler(handleWorkflowQueued)],
            [WebSocketResponseEvents.WORKFLOW_QUEUE_PROCESSED, castHandler(handleWorkflowQueueProcessed)],
        ]
    }
    ...
    async function loadConnectionsFromBackend(): Promise<void> {
        ...
        if (response.connections) {
            connections.value = response.connections.map(connection => normalizeConnection(connection))
        }
    }

    function validateNewConnection(sourcePodId: string | undefined | null, targetPodId: string): boolean {
    ...
    function deleteConnectionsByPodId(podId: string): void {
        ...
        if (selectedConnectionId.value) {
            const stillExists = connections.value.some(connection => connection.id === selectedConnectionId.value)
            if (!stillExists) {
    ...
    function updateDraggingPosition(currentPoint: {x: number; y: number}): void {
    ...
    async function updateConnectionTriggerMode(connectionId: string, triggerMode: TriggerMode): Promise<Connection | null> {
    ...
    function handleWorkflowAutoTriggered(payload: WorkflowHandlerPayload<'handleWorkflowAutoTriggered'>): void {
        getWorkflowHandlers().handleWorkflowAutoTriggered(payload)
    }

    function handleWorkflowAiDecideTriggered(payload: WorkflowHandlerPayload<'handleWorkflowAiDecideTriggered'>): void {
        getWorkflowHandlers().handleWorkflowAiDecideTriggered(payload)
    }

    function handleWorkflowComplete(payload: WorkflowHandlerPayload<'handleWorkflowComplete'>): void {
        getWorkflowHandlers().handleWorkflowComplete(payload)
    }

    function handleWorkflowDirectTriggered(payload: WorkflowHandlerPayload<'handleWorkflowDirectTriggered'>): void {
        getWorkflowHandlers().handleWorkflowDirectTriggered(payload)
    }

    function handleWorkflowDirectWaiting(payload: WorkflowHandlerPayload<'handleWorkflowDirectWaiting'>): void {
        getWorkflowHandlers().handleWorkflowDirectWaiting(payload)
    }

    function handleWorkflowQueued(payload: WorkflowHandlerPayload<'handleWorkflowQueued'>): void {
        getWorkflowHandlers().handleWorkflowQueued(payload)
    }

    function handleWorkflowQueueProcessed(payload: WorkflowHandlerPayload<'handleWorkflowQueueProcessed'>): void {
        getWorkflowHandlers().handleWorkflowQueueProcessed(payload)
    }

    function handleAiDecidePending(payload: WorkflowHandlerPayload<'handleAiDecidePending'>): void {
        getWorkflowHandlers().handleAiDecidePending(payload)
    }

    function handleAiDecideResult(payload: WorkflowHandlerPayload<'handleAiDecideResult'>): void {
        getWorkflowHandlers().handleAiDecideResult(payload)
    }

    function handleAiDecideError(payload: WorkflowHandlerPayload<'handleAiDecideError'>): void {
        getWorkflowHandlers().handleAiDecideError(payload)
    }

    function handleAiDecideClear(payload: WorkflowHandlerPayload<'handleAiDecideClear'>): void {
    ...
    function addConnectionFromEvent(connection: Omit<Connection, 'status'>): void {
        ...
        const exists = connections.value.some(existingConnection => existingConnection.id === enrichedConnection.id)
        if (!exists) {
            connections.value.push(enrichedConnection)
        }
    }

    function updateConnectionFromEvent(connection: Omit<Connection, 'status'>): void {
        const index = connections.value.findIndex(existing => existing.id === connection.id)
        if (index === -1) return
