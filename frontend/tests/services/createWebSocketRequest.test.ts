...
describe('createWebSocketRequest', () => {
  ...
  describe('成功流程', () => {
    it('應該 emit 請求事件並在回應 requestId 匹配時 resolve', async () => {
      ...
      expect(mockOn).toHaveBeenCalledWith('test:response', expect.any(Function))
      ...
      expect(result).toEqual({ requestId: 'test-request-id', result: 'success' })
    })
    ...
    it('應該使用自訂 matchResponse 函數驗證', async () => {
      ...
      expect(result).toEqual({ customId: 'test-request-id', result: 'success' })
    })
  })
  ...
  describe('listener 管理', () => {
    it('應該正確註冊和清理 listener', async () => {
      const config: WebSocketRequestConfig<
        { requestId: string; data: string },
        { requestId: string; result: string }
      > = {
