...
function setupMockSpeechRecognition() {
  ...
  const mockInstance = {
    ...
    continuous: false,
    get onresult() { return onresultHandler },
    set onresult(fn: OnResultHandler | null) { onresultHandler = fn },
  ...
  return { mockStop, mockStart, mockInstance, fireResult: (transcript: string) => {
    if (onresultHandler) {
      onresultHandler({
        results: {
          length: 1,
...
describe('鍵盤行為', () => {
  ...
  it('按下 Ctrl+Enter 應插入換行而非送出（檢查不 emit send）', async () => {
    ...
    editable.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', ctrlKey: true, bubbles: true }))
    await wrapper.vm.$nextTick()
  ...
  it('輸入法組字中（isComposing = true）按 Enter，不應觸發送出', async () => {
    ...
    editable.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', isComposing: true, keyCode: 229, bubbles: true }))
    await wrapper.vm.$nextTick()
...
describe('字數限制', () => {
  ...
  it('輸入文字超過 MAX_MESSAGE_LENGTH 時應被截斷', async () => {
    const wrapper = mountChatInput()
    const editable = wrapper.find('[contenteditable]').element as HTMLDivElement

    const overLimitText = 'a'.repeat(MAX_MESSAGE_LENGTH + 100)
    editable.innerText = overLimitText
    editable.dispatchEvent(new Event('input', { bubbles: true }))
    await wrapper.vm.$nextTick()

    // handleInput 呼叫 updateText 截斷後送出，emitted 的內容不應超過 MAX_MESSAGE_LENGTH
    editable.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }))
    await wrapper.vm.$nextTick()

    const emitted = wrapper.emitted('send')
    if (emitted && emitted.length > 0) {
      const sentText = emitted[0]?.[0]
      expect(typeof sentText).toBe('string')
      expect((sentText as string).length).toBeLessThanOrEqual(MAX_MESSAGE_LENGTH)
    } else {
      // 若超過 MAX_MESSAGE_LENGTH 後送出被攔截，也是合法行為
      expect(wrapper.emitted('send')).toBeFalsy()
    }
    wrapper.unmount()
  })
