...
vi.mock('@/components/ui/select', () => ({
  ...
  SelectValue: { name: 'SelectValue', props: ['placeholder'], template: '<span></span>' },
...
describe('GlobalSettingsModal', () => {
  beforeEach(() => {
    ...
    mockWithErrorToast.mockImplementation((promise: Promise<unknown>) => promise.catch(() => null))
  })

  it('應正確渲染 Modal 標題與兩個模型選擇區塊', async () => {
    mockGetConfig.mockResolvedValue({ success: true, summaryModel: 'sonnet', aiDecideModel: 'sonnet' })
  ...
  it('開啟時應發送 config:get WebSocket 事件載入目前設定', async () => {
    mockWithErrorToast.mockImplementation((promise: Promise<unknown>) => promise)
    mockGetConfig.mockResolvedValue({ success: true, summaryModel: 'opus', aiDecideModel: 'haiku' })
  ...
  it('點擊儲存應發送 config:update WebSocket 事件並傳送正確 payload', async () => {
    ...
    expect(mockUpdateConfig).toHaveBeenCalledWith({ summaryModel: 'sonnet', aiDecideModel: 'sonnet' })
  ...
  it('載入中狀態應禁用儲存按鈕', async () => {
    ...
    resolveGetConfig({ success: true, summaryModel: 'sonnet', aiDecideModel: 'sonnet' })
    await nextTick()
    await nextTick()
