...
describe('ConfirmDeleteModal', () => {
  describe('基本顯示', () => {
    ...
    it('isInUse 為 false 時顯示項目名稱', () => {
      const wrapper = mountModal({ isInUse: false, itemName: '我的 Output Style' })
  ...
  describe('一般類型在使用中', () => {
    ...
    it('subAgent 在 isInUse 為 true 時只顯示確定按鈕', () => {
      ...
      expect(buttons[0]?.text()).toContain('確定')
    })
  ...
  describe('按鈕行為', () => {
    ...
    it('點擊確定按鈕（isInUse 為 true 時）emit update:open 為 false', async () => {
      ...
      const buttons = wrapper.findAll('button')
      const confirmButton = buttons.find(btn => btn.text().includes('確定'))
