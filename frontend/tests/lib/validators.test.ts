...
describe('validators', () => {
  ...
  describe('validateResourceName', () => {
    it('空字串應回傳空值錯誤訊息', () => {
      expect(validateResourceName('', '請輸入名稱', '格式錯誤')).toBe('請輸入名稱')
    })
    ...
    it('包含空格應回傳格式錯誤訊息', () => {
      expect(validateResourceName('my folder', '請輸入名稱', '格式錯誤')).toBe('格式錯誤')
    })

    it('包含中文應回傳格式錯誤訊息', () => {
      expect(validateResourceName('資料夾', '請輸入名稱', '格式錯誤')).toBe('格式錯誤')
    })

    it('合法名稱應回傳 null', () => {
      expect(validateResourceName('my-repo_123', '請輸入名稱', '格式錯誤')).toBeNull()
    })

    it('使用自訂錯誤訊息', () => {
      expect(validateResourceName('', '自訂空值訊息', '自訂格式訊息')).toBe('自訂空值訊息')
      expect(validateResourceName('invalid name', '自訂空值訊息', '自訂格式訊息')).toBe('自訂格式訊息')
    })
  })
