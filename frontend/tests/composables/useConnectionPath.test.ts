...
describe('useConnectionPath', () => {
  describe('calculatePathData', () => {
    ...
    it('midPoint 應在起點和終點之間', () => {
      ...
      expect(result.midPoint.x).toBeLessThanOrEqual(Math.max(startX, endX) + 50)

      // midPoint 的 y 應在 startY 和 endY 之間（考慮 Bezier 曲線可能略超出直線範圍）
      expect(result.midPoint.y).toBeGreaterThanOrEqual(Math.min(startY, endY) - 50)
      expect(result.midPoint.y).toBeLessThanOrEqual(Math.max(startY, endY) + 50)
    })
    ...
    it('不同 anchor 組合應產生不同 control points（top/bottom）', () => {
      ...
      expect(topBottomCoords).toBeDefined()
      expect(bottomTopCoords).toBeDefined()
    ...
    it('不同 anchor 組合應產生不同 control points（left/right）', () => {
      ...
      expect(leftRightCoords).toBeDefined()
      expect(rightLeftCoords).toBeDefined()
  ...
  describe('calculateMultipleArrowPositions', () => {
    ...
    it('每個箭頭應包含 x, y, angle', () => {
      ...
      for (const arrow of result) {
        ...
        expect(arrow).toHaveProperty('y')
        expect(arrow).toHaveProperty('angle')
    ...
    it('calculatePathData 和 calculateMultipleArrowPositions 應使用相同的 control points', () => {
      ...
      const closestArrow = arrows.reduce((closest, arrow) => {
        ...
        const closestDist = Math.sqrt(
          Math.pow(closest.x - midPoint.x, 2) + Math.pow(closest.y - midPoint.y, 2)
        )
      ...
      const distance = Math.sqrt(
        Math.pow(closestArrow.x - midPoint.x, 2) + Math.pow(closestArrow.y - midPoint.y, 2)
      )
