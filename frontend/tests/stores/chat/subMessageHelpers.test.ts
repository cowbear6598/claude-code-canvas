...
import { updateAssistantSubMessages, finalizeSubMessages, flushAndCreateNewSubMessage, appendToolToLastSubMessage, updateSubMessagesToolUseResult, updateMainMessageState } from '@/stores/chat/subMessageHelpers'
...
describe('flushAndCreateNewSubMessage', () => {
  it('應將最後一個 SubMessage 的 isPartial 設為 false', () => {
    ...
    const toolUseInfo: ToolUseInfo = { toolUseId: 'tool-1', toolName: 'Bash', input: {}, status: 'running' }
    const result = flushAndCreateNewSubMessage(subMessages, 'msg-1', toolUseInfo)
...
describe('appendToolToLastSubMessage', () => {
  it('應將 toolUseInfo append 到最後一個 SubMessage 的 toolUse 陣列', () => {
    const toolUse1: ToolUseInfo = { toolUseId: 'tool-1', toolName: 'Bash', input: {}, status: 'running' }
    const toolUse2: ToolUseInfo = { toolUseId: 'tool-2', toolName: 'Read', input: {}, status: 'running' }
    const subMessages: SubMessage[] = [
    ...
    const subMessages: SubMessage[] = [
      {
        ...
        content: '執行中',
        isPartial: false,
        toolUse: [{ toolUseId: 'tool-1', toolName: 'bash', status: 'completed', input: {} }],
      },
      {
        id: 'sub-1',
        content: '',
        isPartial: true,
        toolUse: [{ toolUseId: 'tool-2', toolName: 'read_file', status: 'running', input: {} }],
      },
    ]
    ...
    const subMessages: SubMessage[] = [
      {
        ...
        content: '',
        isPartial: true,
        toolUse: [{ toolUseId: 'tool-1', toolName: 'bash', status: 'running', input: {} }],
      },
    ]
...
describe('updateSubMessagesToolUseResult', () => {
  it('應依據指定 toolUseId 更新對應 tool 的 output 並標記為 completed', () => {
    ...
    const result = updateSubMessagesToolUseResult(subMessages, 'tool-1', '執行結果')
  ...
  it('toolUseId 不存在時不修改任何 tool', () => {
    ...
    const result = updateSubMessagesToolUseResult(subMessages, 'tool-99', '結果')
  ...
  it('多個 subMessages 時只更新包含該 toolUseId 的', () => {
    ...
    const result = updateSubMessagesToolUseResult(subMessages, 'tool-2', '讀取結果')
...
describe('updateMainMessageState', () => {
  ...
  it('有 toolUse 時應更新 message 的 toolUse', () => {
    ...
    const updatedToolUse: ToolUseInfo[] = [
      { toolUseId: 'tool-1', toolName: 'bash', status: 'completed', input: {}, output: '結果' },
    ]

    const result = updateMainMessageState(message, 'Hello', updatedToolUse, undefined)
  ...
  it('有 subMessages 時應更新 message 的 subMessages', () => {
    ...
    const result = updateMainMessageState(message, 'Hello', undefined, finalizedSubMessages)
  ...
  it('toolUse 為 undefined 時不應覆蓋原本的 toolUse', () => {
    ...
    const result = updateMainMessageState(message, 'Hello', undefined, undefined)
  ...
  it('應將 isPartial 設為 false 並更新 content', () => {
    ...
    const result = updateMainMessageState(message, '新內容', undefined, undefined)
