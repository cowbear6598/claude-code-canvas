...
describe('selectionStore', () => {
  ...
  describe('getters', () => {
    ...
    describe('selectedOutputStyleNoteIds', () => {
      it('應篩選出 type 為 outputStyleNote 的 id', () => {
        const store = useSelectionStore()
        store.selectedElements = [
          { type: 'pod', id: 'pod-1' },
          { type: 'outputStyleNote', id: 'note-1' },
          { type: 'outputStyleNote', id: 'note-2' },
          { type: 'skillNote', id: 'note-3' },
        ]

        expect(store.selectedOutputStyleNoteIds).toEqual(['note-1', 'note-2'])
      })
    })
    ...
    describe('isElementSelected', () => {
      it('元素在 selectedElements 中時應回傳 true', () => {
        ...
        expect(store.isElementSelected('outputStyleNote', 'note-1')).toBe(true)
      })
      ...
      it('元素不在 selectedElements 中時應回傳 false', () => {
        ...
        expect(store.isElementSelected('outputStyleNote', 'note-1')).toBe(false)
      })
      ...
      it('應依 type 和 id 同時判斷', () => {
        ...
        expect(store.isElementSelected('pod', 'pod-1')).toBe(true)
        expect(store.isElementSelected('outputStyleNote', 'pod-1')).toBe(false)
      })
    })
  })
  ...
  describe('calculateSelectedElements', () => {
    ...
    describe('Note 選取', () => {
      it('OutputStyleNote 與框選範圍相交時應被選中', () => {
        const store = useSelectionStore()
        const note = createMockNote('outputStyle', { id: 'note-1', x: 100, y: 100 })
      ...
      it('RepositoryNote 與框選範圍相交時應被選中', () => {
        const store = useSelectionStore()
        const note = createMockNote('repository', { id: 'note-1', x: 100, y: 100 })
      ...
      it('SubAgentNote 與框選範圍相交時應被選中', () => {
        const store = useSelectionStore()
        const note = createMockNote('subAgent', { id: 'note-1', x: 100, y: 100 })
      ...
      it('CommandNote 與框選範圍相交時應被選中', () => {
        const store = useSelectionStore()
        const note = createMockNote('command', { id: 'note-1', x: 100, y: 100 })
      ...
      it('Note 與框選範圍無相交時不應被選中', () => {
        const store = useSelectionStore()
        const note = createMockNote('outputStyle', { id: 'note-1', x: 500, y: 500 })
    ...
    describe('混合選取', () => {
      it('應能同時選取 Pod 和多種 Note', () => {
        const store = useSelectionStore()
        const pod = createMockPod({ id: 'pod-1', x: 100, y: 100 })
        const outputNote = createMockNote('outputStyle', { id: 'note-1', x: 150, y: 150 })
        const skillNote = createMockNote('skill', { id: 'note-2', x: 200, y: 200 })
        const repoNote = createMockNote('repository', { id: 'note-3', x: 250, y: 250 })
      ...
      it('應正確過濾不在範圍內的元素', () => {
        ...
        const note1 = createMockNote('outputStyle', { id: 'note-1', x: 150, y: 150 })
        const note2 = createMockNote('outputStyle', { id: 'note-2', x: 2000, y: 2000 })
  ...
  describe('toggleElement', () => {
    ...
    it('未選取的元素應被加入', () => {
      ...
      store.toggleElement({ type: 'outputStyleNote', id: 'note-1' })
    ...
    it('應依 type 和 id 同時判斷', () => {
      ...
      store.toggleElement({ type: 'outputStyleNote', id: 'pod-1' })

      expect(store.selectedElements).toEqual([
        { type: 'pod', id: 'pod-1' },
        { type: 'outputStyleNote', id: 'pod-1' },
      ])
    })
