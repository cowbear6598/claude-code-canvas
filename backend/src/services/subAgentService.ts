...
class SubAgentService {
  ...
  async listSubAgents(): Promise<SubAgent[]> {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(`[SubAgentService] Failed to read agent ${agentId}:`, error);
      }
    }
  ...
  async copySubAgentToPod(subAgentId: string, podId: string): Promise<void> {
    ...
    console.log(`[SubAgentService] Successfully copied agent ${subAgentId} to pod ${podId}`);
  }
  ...
  async copySubAgentToRepository(subAgentId: string, repositoryPath: string): Promise<void> {
    ...
    await fs.mkdir(path.dirname(destPath), { recursive: true });
    await fs.copyFile(srcPath, destPath);
    console.log(`[SubAgentService] Successfully copied agent ${subAgentId} to repository at ${repositoryPath}`);
  }
  ...
  async deleteSubAgentsFromPath(basePath: string): Promise<void> {
    ...
    try {
      ...
      console.log(`[SubAgentService] Deleted agents directory at ${agentsDir}`);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
        console.warn(`[SubAgentService] Failed to delete agents directory at ${agentsDir}:`, error);
      }
    }
  }
  ...
  async delete(subAgentId: string): Promise<void> {
    ...
    console.log(`[SubAgentService] Deleted agent file: ${subAgentId}.md`);
  }
