...
describe('斷線重連行為', () => {
    beforeEach(() => {
        ...
        asMock(claudeQueryService.abortQuery).mockClear();
    ...
    describe('斷線後活躍查詢不被中斷', () => {
        it('呼叫 cleanupSocket 不會觸發 abortQuery', async () => {
            ...
            // 啟動查詢
            const queryPromise = claudeQueryService.sendMessage(podId, 'test message', vi.fn());

            // 模擬斷線清理（只呼叫 cleanupSocket，不應連帶 abort 查詢）
            socketService.cleanupSocket(connectionId);

            // 驗證查詢未被中斷
            expect(claudeQueryService.abortQuery).not.toHaveBeenCalled();
    ...
    describe('重連後以新 connectionId 發送停止請求應成功', () => {
        it('以不同 connectionId 發送 abort 請求仍能成功中斷查詢', () => {
            ...
            expect(claudeQueryService.abortQuery).toHaveBeenCalledWith(podId);
        ...
        it('連線 A 斷線後，連線 B 可以發送 abort 請求', () => {
            ...
            const abortedByB = claudeQueryService.abortQuery(podId);
